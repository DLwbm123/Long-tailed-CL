...
This document defines the single-head GPA reproduction scaffold. GPA has no available public training implementation to audit, so the paper is treated as the method specification and GVAlign / Long-Tailed-CIL is treated as the data and protocol reference.
...
| dynamic anchor | Eq. 5 | current-task moving-average centroid initialized from frozen prototypes; classifier rows are regularized toward normalized EMA centroids | paper says moving-average centroid, exact update timing is not code-specified |
| anchor loss | Eq. 5 | L2 squared sum between classifier rows and normalized current-task EMA centroids | exact reduction not code-specified |
...
4. Dynamic anchor is implemented as a moving-average target per current-task class. The first target is the frozen prototype. The anchor loss is applied to classifier rows, not directly to feature centroids.
...
7. This scaffold is not GVAlign multi-head GPA. It should not inherit the failed multi-head plugin conclusions except as a motivation for single-head reproduction.
