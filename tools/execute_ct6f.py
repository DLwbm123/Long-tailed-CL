...
def main():
    ...
    for mode in ('train','evaluate','report'):
        ...
        with (r/'output'/f'{mode}.log').open('xb') as f:
            try:code=subprocess.run(['/tmp/n154env/bin/python','-u','/tmp/p25r.py' if mode=='report' else '/tmp/p25.py'],env=env,stdin=subprocess.DEVNULL,stdout=f,stderr=f,timeout=600 if mode=='report' else None).returncode
