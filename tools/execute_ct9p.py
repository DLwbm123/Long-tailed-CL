...
def main():
    r=Path('/tmp/p28root');pub=r/'output/public';read=lambda name:json.loads((pub/name).read_text())
    ...
    env=dict(os.environ,P28_CONFIG=str(r/'config.json'),PYTHONDONTWRITEBYTECODE='1',OMP_NUM_THREADS='4',MKL_NUM_THREADS='4',OPENBLAS_NUM_THREADS='4',CUDA_VISIBLE_DEVICES='0');receipts=[]
    for mode in ('qualify','train','evaluate','report'):
        env['P28_MODE']=mode
        if mode=='report':env['CUDA_VISIBLE_DEVICES']=''
        t=time.time()
        with (r/'output'/f'{mode}.log').open('xb') as f:
            try:code=subprocess.run(['/tmp/n154env/bin/python','-u','/tmp/p28r.py' if mode=='report' else '/tmp/p28.py'],env=env,stdin=subprocess.DEVNULL,stdout=f,stderr=f,timeout=600 if mode=='report' else None).returncode
