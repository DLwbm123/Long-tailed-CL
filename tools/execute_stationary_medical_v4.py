...
def verify(config):
    ...
    if config.get('candidate_branch') in ('H','I'):
    ...
    if config.get('candidate_branch') in ('H','I'):assert json.loads((Path(config['v5_output'])/'FINAL_STATUS.json').read_text())['status']=='COMPLETE'
