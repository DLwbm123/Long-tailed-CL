...
def main():
    ...
    for directory,units in [(root/'output',read(pub/'PREDICTIONS_LOCK.json')['units']),(old,references)]:
        for e in units:
    ...
    assert sum(x['method']=='F1' for x in rows)==45 and sum(x['method']=='F1' for x in pcs)==459
    out=[];audit=[]
    for ds,last in [('HK',11),('ISIC',4)]:
        ...
        w=bootstrap_weights(canon,2000,49001,sorted(set(canon['original'])));boot={}
        for key,p in data.items():
            if key[0]!=ds:continue
    ...
    for filename,table in [('metrics',[r for r in rows if r['method']=='F1']),('per_class',[r for r in pcs if r['method']=='F1']),('comparison_metrics',rows),('comparison_per_class',pcs),('paired_all_metrics',out),('final_metrics',final),('forgetting',forget),('errors',errors)]:csvwrite(pub/(filename+'.csv'),table)
    text=['# CT5-F 完整任务范围冻结Task1参照','', '沿用三种顺序，HK为完整23类11任务，ISIC为完整8类4任务。正常两类Task1之后冻结，新增神经训练epoch/step=0；仅val，test/reserved访问=0。','', '| 数据 | 方法 | Final BA均值 | 最差seed | 全任务平均BA |','|---|---|---:|---:|---:|']
    for ds in ('HK','ISIC'):
        for method in ('F1','P','C0'):
    ...
    text+=['','以上为观察结果，不能据此前缀外推FD全程效果。区间只条件于现有模型和val component；未重采样seed，P终端是确定性参照，singleton和反复使用val的限制仍在，多重比较未校正。18个前缀预测原样复用，27个新增阶段；为恢复未保存的充分统计而顺序重新提取Task2/3当期fit，不将旧fit作为回放。','', '所有逐阶段/逐类、old/current/HM/tail、零召回、误分及遗忘见配套CSV。NEXT_DECISION=STOP。']
    (pub/'FINAL_REPORT_ZH.md').write_text('\n'.join(text))
    elapsed=time.monotonic()-start;assert elapsed<1800
    (pub/'ANALYSIS_AUDIT.json').write_text(json.dumps(dict(status='PASS',saved_scores_only=True,new_forwards=0,new_fits=0,rows=135,per_class=1377,bootstrap_n=2000,seed=49001,CPU_report_seconds=elapsed,checks=audit),indent=2))
    (pub/'NEXT_DECISION.md').write_text('STOP\n完整固定矩阵完成；不自动扩展本轮。\n')
    (pub/'COMPLETE.json').write_text(json.dumps(dict(status='COMPLETE_CT5F',NEXT_DECISION='STOP',units=45,per_class=459,neural_epochs=0,optimizer_steps=0,test_predictions=0),indent=2))
