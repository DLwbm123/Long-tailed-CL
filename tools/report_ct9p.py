...
def main():
    ...
    assert len(rows)==30 and len(pcs)==240
    contrasts=[('B1T','C3'),('B1A','C2'),('B1T','F1'),('B1A','F1'),('B1T','B1A')];paired=[]
    for ds in ('HK','ISIC'):
        identity={}
        ...
        weights=bootstrap_weights(canon,2000,52001,sorted(set(canon['original'])));boot={}
        for key,p in data.items():
            if key[0]==ds:
    ...
    report=['# CT9-P Task4系数干预','', '仅Task4 beta由10改1，继承原beta10的Task1–3，不是低FD全程训练。HK为8/23类前缀；ISIC为8类。全部是val开发性比较，test访问0。','', '|数据|方法|Task4 BA|最差seed|old|current|tail|','|---|---|---:|---:|---:|---:|---:|']
    for ds in ('HK','ISIC'):
    ...
    report+=['','2000次seed52001按类component配对区间，固定模型，不重抽seed，多重比较未校正。HK各seed前8类不同，配对只在同seed同布局，不能当同一八类的三次训练；不把结果外推完整23类或其他FD强度。NEXT_DECISION=STOP。']
    (pub/'FINAL_REPORT_ZH.md').write_text('\n'.join(report));(pub/'NEXT_DECISION.md').write_text('STOP\n固定六配对Task4干预完成，不扩展任务或搜索beta。\n')
    ledger=read(pub/'RESOURCE_LEDGER.json');assert ledger['calls']['formal_task_epochs']==60 and ledger['calls']['formal_optimizer_steps']==2750
    (pub/'ANALYSIS_AUDIT.json').write_text(json.dumps(dict(status='PASS',rows=30,per_class=240,new_forwards=0,new_fits=0,bootstrap_n=2000,bootstrap_seed=52001,CPU_seconds=time.monotonic()-start),indent=2))
    (pub/'COMPLETE.json').write_text(json.dumps(dict(status='COMPLETE_CT9P',NEXT_DECISION='STOP',neural_epochs=60,steps=2750,new_checkpoints=6,new_prediction_units=12,test_predictions=0),indent=2))
