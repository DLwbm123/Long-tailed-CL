...
"""
    ...
    text+=f"每条增量学习器的继续更新状态最多约 {res['continued_increment_bytes_max']/1024**2:.2f} MiB；"+'审计归档额外保留11份去重最终累计统计、一个活跃状态、各阶段 W、私有 validation 分数和统计来源。没有保存全量合成特征。原缓存是既有资产依赖；本次运行的总峰值 RSS 包括审计/诊断工作空间，不把它伪称为学习器“零存储”。这些统计/特征不宣称隐私安全。公开内容仅源码、协议和聚合审计表；个体特征、身份关联、精确私有路径和分数保留私有。\n\n'
