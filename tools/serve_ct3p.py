            ...
            if key not in done:
                ...
                try:
                    ...
                    if op=='GET':
                        ...
                        subprocess.run(ssh+['/tmp/n154env/bin/python /tmp/p22io.py receive'],input=zstandard.ZstdCompressor(level=3,threads=4).compress(p.read_bytes()),check=True,timeout=180)
                        ...
                        data=subprocess.check_output(ssh+['/tmp/n154env/bin/python /tmp/p22io.py send'],timeout=180);
