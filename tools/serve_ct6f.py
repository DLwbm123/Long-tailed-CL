...
def main():
    ...
    ssh=['ssh','-S','/tmp/q22-transfer2','-o','BatchMode=yes','-p',os.environ.get('P25_SSH_PORT','30128'),'root@hb01-ssh.gpuhome.cc']
            ...
            if key not in done:
                ...
                try:
                    ...
                    if op=='GET':
                        ...
                        subprocess.run(ssh+['/tmp/n154env/bin/python /tmp/p25io.py receive'],input=zstandard.ZstdCompressor(level=3,threads=4).compress(p.read_bytes()),check=True,timeout=180)
                        ...
                        data=subprocess.check_output(ssh+['/tmp/n154env/bin/python /tmp/p25io.py send'],timeout=180);
