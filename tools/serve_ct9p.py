...
def main():
    ...
    root=Path(os.environ['P28_ARCHIVE']);old=Path('/tmp/p22archive');root.mkdir(parents=True,exist_ok=True)
    assert os.stat(root).st_dev!=os.stat('/').st_dev
    p=root/'.probe';p.write_text('ready');assert p.read_text()=='ready';p.unlink()
    ssh=['ssh','-S','/tmp/q25-transfer-r1','-o','BatchMode=yes','-o','ConnectTimeout=10','-o','ServerAliveInterval=15','-o','ServerAliveCountMax=2','-p',os.environ.get('P28_SSH_PORT','30128'),'root@hb01-ssh.gpuhome.cc']
    def send(x):subprocess.run(ssh+['cat > /tmp/p28root/output/private/ACK.json.part && mv /tmp/p28root/output/private/ACK.json.part /tmp/p28root/output/private/ACK.json'],input=json.dumps(x).encode(),check=True,timeout=30)
    oldnames={f'{n}_{s}_F_t{t:02d}.pt' for n in ('HK','ISIC') for s in (1993,1994,1995) for t in (3,)}
    newnames={f'{n}_{s}_F_t{t:02d}.pt' for n in ('HK','ISIC') for s in (1993,1994,1995) for t in (4,)}
    done=set()
    while True:
        q=subprocess.run(ssh+['cat /tmp/p28root/output/private/REQUEST.json'],capture_output=True,timeout=30)
        if q.returncode==0:
            ...
            if key not in done:
                ...
                try:
                    ...
                    if op=='GET':
                        ...
                        subprocess.run(ssh+['/tmp/n154env/bin/python /tmp/p28io.py receive'],input=zstandard.ZstdCompressor(level=3,threads=4).compress(p.read_bytes()),check=True,timeout=600)
                        ...
                        p=root/(name+'.part')
                        assert sum(f.stat().st_size for f in root.iterdir() if f.is_file())+req['bytes']<3*1024**3
                        data=subprocess.check_output(ssh+['/tmp/n154env/bin/python /tmp/p28io.py send'],timeout=600);
        ...
        stop=subprocess.run(ssh+['test -f /tmp/p28root/output/private/STOP_TRANSFER.json'],capture_output=True,timeout=30)
